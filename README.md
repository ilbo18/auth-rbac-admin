...
JWT 인증, 역할 기반 접근 제어(RBAC), 메뉴 단위 권한 관리, 감사 로그를 한 흐름으로 묶어 관리자 시스템에서 자주 필요한 인증/인가 문제를 직접 구현하고 검증하는 데 목적이 있습니다.
...
phase 3 정리 이후에는 bootstrap 공개 API를 두지 않습니다.
...
이 방식이 공개 API를 열어두는 방식보다 설명이 단순하고, 보안 정책도 일관되게 유지할 수 있습니다.
